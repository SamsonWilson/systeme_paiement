...
class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # Obtention de l'utilisateur et de la chambre
        location = get_object_or_404(Chambre, id=location_id)  # noqa: F841
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        location_id = self.kwargs.get('location_id')
        context['location'] = get_object_or_404(Chambre, id=location_id)
        return context

    def get_success_url(self):
        # En utilisant reverse avec un pattern d'URL dynamique pour générer le PDF
        return reverse('generate_pdf', kwargs={'paiementloyer_id': self.object.id})
# 
# 
# 
# class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
