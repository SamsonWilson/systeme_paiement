...
class PDFTemplateViewpaiyement1(View):
   """
    Vue pour générer et servir directement un PDF de reçu de paiement avec WeasyPrint.
    """
   def get(self, request, paiement_id, *args, **kwargs):try:
    # On récupère le paiement depuis la base
    paiement = get_object_or_404(PaiementLoyer, id=paiement_id)
            ...
            print(f"Erreur lors de la génération du PDF pour paiement {paiement_id}: {e}")
            return HttpResponse(f"Erreur lors de la génération du PDF: {e}", status=500)
...
class Download_Payement1_RedirectView(View):
    ...
    def get(self, request, location_id, *args, **kwargs):
        # On prépare les URLs dont le JavaScript aura besoin
        context = {
            'pdf_url': reverse('generate_pdfpayement', args=[location_id]),
            'home_url': reverse('liste_paiements')  # Assurez-vous d'avoir une URL nommée 'accueil'
        }
        return render(request, 'messageDeSucce/succe_Location.html', context)
