...
class FinLocationCreateView(CreateView):
    ...
    success_url = reverse_lazy('fin_location_list')  # Redirige vers la liste des FinLocation après succès

    def form_valid(self, form):
        # Vous pouvez ajouter des validations personnalisées avant de sauvegarder
        return super().form_valid(form)
...
class ListePaiementsLocationView(LoginRequiredMixin, ListView): # Ajout de LoginRequiredMixin (optionnel)
        """
        Affiche la liste paginée des paiements de loyer pour une location spécifique,
        inclut les détails de la location et le montant total payé.
        """
        model = PaiementLoyer
        # Assurez-vous que ce chemin est correct par rapport à votre structure de templates
        template_name = 'accuiel/Finlocation/listePaiement.html'
        context_object_name = 'paiements'  # Liste des paiements pour la page actuelle
        paginate_by = 10                   # Active la pagination

/*************  ✨ Windsurf Command 🌟  *************/
        def get_queryset(self):
            """
            Retourne le queryset filtré et ordonné des PaiementLoyer
            pour la location spécifiée dans l'URL.
            Note : Cette méthode est principalement pour la ListView de base (pagination).
                L'objet Location lui-même sera récupéré dans get_context_data.

            :return: un queryset filtré et ordonné des PaiementLoyer
            """
            location_id = self.kwargs.get('location_id')

            # Il est bon de vérifier que la location existe, même si on ne stocke pas l'objet ici
            # Si la location n'existe pas, la page 404 sera affichée
            get_object_or_404(Location, pk=location_id)

            # Filtre les paiements en utilisant directement l'ID de la location
            # Ordonne les paiements par date de paiement la plus récente
            queryset = PaiementLoyer.objects.filter(location_id=location_id).order_by('-date_paiement')
            return queryset
/*******  e6b7307f-54e4-4f4f-bb58-aea23624dc3a  *******/

        def get_context_data(self, **kwargs):
            """
            Ajoute au contexte :
            1. L'objet Location spécifique (avec related fields préchargés).
            2. Le montant total payé pour cette location.
            3. Le titre de la page.
            """
            # Récupère le contexte de base (inclut 'paiements' paginés, 'page_obj', etc.)
            context = super().get_context_data(**kwargs)
            location_id = self.kwargs.get('location_id')

            # --- Récupération optimisée de l'objet Location ---
            # On récupère l'objet Location UNE SEULE FOIS ici, en préchargeant
            # les données liées nécessaires pour le template via select_related.
            location_obj = get_object_or_404(
                Location.objects.select_related(
                    'utilisateur',        # Pour le nom du locataire
                    'chambre',            # Pour les détails de la chambre
                    'chambre__maison',    # Pour le nom et l'adresse de la maison
                    'chambre__type_chambre' # Pour le type de chambre
                ),
                pk=location_id
            )
            # Ajoute l'objet location complet au contexte
            context['location'] = location_obj

            # --- Calcul du total des paiements ---
            # On calcule la somme sur TOUS les paiements de cette location (pas seulement la page actuelle)
            # On utilise l'objet 'location_obj' qu'on vient de récupérer pour filtrer
            payment_stats = PaiementLoyer.objects.filter(location=location_obj).aggregate(
                total_paid=Sum('montant_paye')
            )
            # Ajoute la somme au contexte, en gérant le cas où il n'y a aucun paiement (None)
            context['total_paid_amount'] = payment_stats.get('total_paid') or Decimal('0.00')

            # --- Titre de la page ---
            context['titre_page'] = f"Historique des Paiements - Location #{location_obj.id}"
