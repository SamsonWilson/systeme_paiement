...
class ListePaiementsLocationView(LoginRequiredMixin, ListView): # Ajout de LoginRequiredMixin (optionnel)
    """
    Affiche la liste des paiements de loyer pour une location spécifique.
    """
    model = PaiementLoyer
    template_name = 'votre_app/liste_paiements_location.html' # Nom du fichier template
    context_object_name = 'paiements' # Nom de la variable dans le template
    paginate_by = 10 # Optionnel: Ajoutez de la pagination si la liste peut être longue

    def get_queryset(self):
        """
        Filtre les paiements pour ne retourner que ceux
        de la location spécifiée dans l'URL.
        """
        # Récupère l'ID de la location depuis les arguments de l'URL (kwargs)
        location_id = self.kwargs.get('location_id')
        # Récupère l'objet Location (ou lève une erreur 404 s'il n'existe pas)
        self.location = get_object_or_404(Location, pk=location_id)
        # Filtre les paiements liés à cette location et les ordonne
        queryset = PaiementLoyer.objects.filter(location=self.location).order_by('-date_paiement')
        return queryset

    def get_context_data(self, **kwargs):
        """
        Ajoute l'objet Location au contexte pour pouvoir afficher
        ses informations dans le template (ex: nom du locataire).
        """
        context = super().get_context_data(**kwargs)
        # Ajoute l'objet location (récupéré dans get_queryset) au contexte
        # Récupérons la location avec select_related pour l'efficacité dans le template
        context['location'] = get_object_or_404(
            Location.objects.select_related('utilisateur', 'chambre', 'chambre__maison'),
            pk=self.kwargs.get('location_id')
        )
        context['titre_page'] = f"Historique des Paiements pour la Location #{context['location'].id}"
        return context
