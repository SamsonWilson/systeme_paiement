...
class PaiementLoyer2CreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form_kwargs(self):
        ...
        location = self.get_location_object()
        if location:
            kwargs['location'] = location  # Doit être géré dans le constructeur du form
        return kwargs
    ...
    def get_context_data(self, **kwargs):
        ...
        if paiement:
            context['location'] = paiement
            context['montant_mensuel'] = paiement.montant_total
            context['date_fin_avance_iso'] = paiement.date_fin_avance.isoformat() if location.date_fin_avance else None
        return context
