...
class PDFTemplateViewLocation(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        facture = {
            ...
            "total": 125.50,
            "lignes": [
                {"designation": "Produit A", "prix": 50},
                {"designation": "Produit B", "prix": 75.5},
            ]
        }

        # Render du template
        html_string = render_to_string("recu/recu_Location.html", {"facture": facture})

        # Crée la réponse HTTP avec le bon header
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = f'inline; filename="facture_{facture_id}.pdf"'
