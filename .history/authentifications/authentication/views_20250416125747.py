...
class PaiementLoyerListView(TemplateView):
    template_name = 'accueil/paiements/paiement_list.html'
    paginate_by = 15
    ...
    def get_context_data(self, **kwargs):
        ...
        # On récupère toutes les locations
        locations = Location.objects.select_related('chambre', 'utilisateur')
        
        for location in locations:
            # Dernier paiement pour chaque location
            dernier_paiement = PaiementLoyer.objects.filter(location=location).order_by('-date_paiement').first()

            date_fin = dernier_paiement.date_fin_paiement if dernier_paiement else None
            date_expiree = date_fin < current_date_togo.date() if date_fin else False
        ...
        # Trier les paiements par date de fin (optionnel)
        paiements_par_location.sort(key=lambda x: x['date_fin'] or current_date_togo.date())
        ...
        context.update({
            ...
            'date_actuelle_togo': current_date_togo,
            'total_paiements': PaiementLoyer.objects.count(),
