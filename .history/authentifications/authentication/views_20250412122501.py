...
class PaiementLoyerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        """
        Affiche le dernier paiement pour chaque location avec mise en surbrillance si expiré.
        """
        context = super().get_context_data(**kwargs)
        paiements_par_location = []
        current_date_togo = datetime.now(pytz.timezone('Africa/Lome'))

        # Récupère toutes les locations (tu peux filtrer sur les actives si besoin)
        locations = Location.objects.select_related('chambre', 'locataire')

        for location in locations:
            paiements = PaiementLoyer.objects.filter(location=location).order_by('-date_paiement')
            dernier_paiement = paiements.first()
            ...
            paiements_par_location.append({
                ...
                'chambre': location.chambre,
                 'locataire': location.utilisateur,  # Remplacé ici
