...
class GetNewMessagesView(View):
    def get(self, request, maison_id):
        last_id = int(request.GET.get("last_id", 0))
        new_messages = Message.objects.filter(maison_id=maison_id, id__gt=last_id).select_related("sender")
            ...
            "new_messages": [
                {
                    ...
                    "sender_name": getattr(m.sender, "prenom", m.sender.username),
                    "text": m.message,
...
from django.http import JsonResponse
from django.views import View
from .models import Message, Maison

class CheckNewMessagesView(View):
    def get(self, request, maison_id):
        # On récupère l'ID du dernier message que le client connaît
        last_message_id = request.GET.get('last_message_id', 0)
        
        # On cherche tous les messages plus récents dans cette maison
        new_messages = Message.objects.filter(
            maison_id=maison_id,
            id__gt=last_message_id
        ).order_by('timestamp')

        # On prépare les données à renvoyer en JSON
        messages_data = []
        for message in new_messages:
            messages_data.append({
                ...
                'sender': {
                    ...
                    'prenom': message.sender.prenom or message.sender.username,
                },
...
from .models import Maison, Notification
...
class InboxView(ListView):
    ...
    def get_queryset(self):
        # Récupère les maisons pertinentes pour l'utilisateur
        # Par exemple, toutes les maisons s'il est admin
        if self.request.user.is_staff:
            return Maison.objects.all().order_by('nom')
        # Ou seulement les maisons où il est locataire
        # (cette logique est à adapter à ton application)
        else:
             # Exemple: renvoie les maisons où l'utilisateur a une location active
            return Maison.objects.filter(chambres__locations__utilisateur=self.request.user).distinct()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        user = self.request.user

        # Pour chaque maison, on calcule le nombre de notifications non lues
        maisons_with_unread = []
        for maison in context['maisons']:
            unread_count = Notification.objects.filter(
                destinataire=user,
                message__maison=maison,
                lu=False
            ).count()
            maison.unread_count = unread_count # On ajoute l'attribut
            maisons_with_unread.append(maison)
        
        context['maisons'] = maisons_with_unread
        return context
...
class MaisonLocatairesView(LoginRequiredMixin, View):
    ...
    def get(self, request, maison_id):
        ...
        # ✅ Locataires rattachés à la maison
