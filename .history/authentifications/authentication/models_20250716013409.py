...
#     # Champs pour chambre simple
#     longueur = models.FloatField(blank=True, null=True)
#     largeur = models.FloatField(blank=True, null=True)

#     # Champs pour chambre salon
#     chambre_longueur = models.FloatField(blank=True, null=True)
#     chambre_largeur = models.FloatField(blank=True, null=True)
/*************  ✨ Windsurf Command 🌟  *************/
    # Champs pour chambre salon
    # La longueur et la largeur du salon sont stockées séparément
    # pour permettre de calculer la surface totale de la chambre
    # salon correctement.
    salon_longueur = models.FloatField(
        blank=True,
        null=True,
        help_text="Longueur du salon (en mètres)"
    )
    salon_largeur = models.FloatField(
        blank=True,
        null=True,
        help_text="Largeur du salon (en mètres)"
    )
#     salon_longueur = models.FloatField(blank=True, null=True)
#     salon_largeur = models.FloatField(blank=True, null=True)
/*******  b034b66b-f535-43f1-adef-430b9f45fe79  *******/
...
#     etat = models.CharField(max_length=20, choices=ETAT_CHOICES, default='libre')
#     description = models.TextField(blank=True, null=True)
...
#         type_nom = self.type_chambre.nom.lower() if self.type_chambre and self.type_chambre.nom else ""

#         if type_nom == "chambre simple" and self.longueur and self.largeur:
#             self.surface = self.longueur * self.largeur

#         elif type_nom == "chambre salon" and all([
#             self.chambre_longueur, self.chambre_largeur,
#             self.salon_longueur, self.salon_largeur
#         ]):
#             surface_chambre = self.chambre_longueur * self.chambre_largeur
#             surface_salon = self.salon_longueur * self.salon_largeur
#             self.surface = surface_chambre + surface_salon

#         else:
#             self.surface = None  # ou 0.0 si tu préfères

#         super().save(*args, **kwargs)

#     def __str__(self):
#         return f"{self.type_chambre.nom} - {self.etat} - {self.surface or '??'} m²"
