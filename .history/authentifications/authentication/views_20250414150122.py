...
class PaiementLoyer2CreateView(LoginRequiredMixin, CreateView):
    ...
    model = PaiementLoyerform_class = PaiementLoyerForm  # Utilisez le formulaire personnalisé ou laissez Django générer un formulaire automatique
        template_name = 'paiement_loyer_form.html'  # Nom du template pour le formulaire
        success_url = reverse_lazy('nom_de_votre_url_de_succès')  # Redirection après succès

        def form_valid(self, form):
            # Associer automatiquement la location au paiement via l'ID de l'URL
            location_id = self.kwargs.get('location_id')
            form.instance.location = Location.objects.get(id=location_id)
            return super().form_valid(form)
    # def get_location_object(self):
    #     """Helper method to retrieve the Location object."""
    #     location_id = self.kwargs.get('location_id')
