...
# forms.py
from django import forms
from .models import Message, Maison, CustomUser

class AdminMessageForm(forms.ModelForm):
    class Meta:
        model = Message
        fields = ['maison', 'group_receivers', 'message', 'image', 'is_group_message']

    def __init__(self, *args, **kwargs):
        user = kwargs.pop('user')
        super().__init__(*args, **kwargs)
        # Restreindre les maisons à celles qui ont des locataires actifs
        maisons_ids = Maison.objects.filter(
            chambres__locations__utilisateur__type_utilisateur__nom__iexact='locataire'
        ).distinct().values_list("id", flat=True)
        self.fields['maison'].queryset = Maison.objects.filter(id__in=maisons_ids)
        self.fields['group_receivers'].queryset = CustomUser.objects.none()  # Initialement vide

        # Si une maison est déjà sélectionnée
        if 'maison' in self.data:
            try:
                maison_id = int(self.data.get('maison'))
                maison = Maison.objects.get(id=maison_id)
                self.fields['group_receivers'].queryset = maison.get_locataires_actifs()
            except (ValueError, Maison.DoesNotExist):
                pass
        elif self.instance.pk and self.instance.maison:
            self.fields['group_receivers'].queryset = self.instance.maison.get_locataires_actifs()
# dans views.py
# dans views.py
...
class MaisonMessageView(LoginRequiredMixin, View):
    ...
    def get(self, request, maison_id):
        ...
        if not request.user.is_staff and maison not in request.user.get_maisons_louees():
        ...
        context = {'maison': maison, 'messages': messages}
