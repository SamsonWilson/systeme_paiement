...
class ChatView(LoginRequiredMixin, View):
    def get(self, request, receiver_id=None, maison_id=None):
        """
        Gère l'affichage des messages individuels ou de groupe.
        """
        user = request.user
        # maisons = Maison.objects.all()
        # Si un ID de maison est fourni, charger les messages de groupe
        if maison_id:
            maison = get_object_or_404(Maison, id=maison_id)
            messages = Message.objects.filter(maison=maison, is_group_message=True).order_by('timestamp')
        ...
        # Si aucun destinataire ou maison n'est spécifié
        raise Http404("Aucun destinataire ou maison spécifié.")

    def post(self, request, receiver_id=None, maison_id=None):
        """
        Gère l'envoi des messages individuels ou de groupe.
        """
        user = request.user
        ...
        if maison_id:
            ...
            # Ajouter tous les utilisateurs de la maison comme destinataires
            group_receivers = CustomUser.objects.filter(maison=maison).exclude(id=user.id)
            message.group_receivers.set(group_receivers)
            return JsonResponse({
        ...
        # Si aucune cible n'est spécifiée
        return JsonResponse({"message": "Aucun destinataire ou maison spécifié"}, status=400)
# class GroupMessageView(LoginRequiredMixin, View):
#     def get(self, request):
#         form = GroupMessageForm(user=request.user)
#         return render(request, 'accuiel/messages/group_message_form.html', {
#             'form': form,
#         })

#     def post(self, request):
#         form = GroupMessageForm(request.POST, user=request.user)
#         if form.is_valid():
...
#             # Ajouter les destinataires au groupe (tous les utilisateurs liés à la maison sauf l'expéditeur)
#             locataires = maison.locataires.exclude(user=sender)
#             for locataire in locataires:
#                 group_message.group_receivers.add(locataire.user)

#             # Si le propriétaire n'est pas l'expéditeur, l'ajouter aussi
#             if maison.proprietaire.user != sender:
#                 group_message.group_receivers.add(maison.proprietaire.user)

#             return JsonResponse({"message": "Message de groupe envoyé avec succès"})
...
from .models import Message, CustomUser, Maison
...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Récupérer tous les utilisateurs pour la barre latérale
        all_users = CustomUser.objects.exclude(id=user.id)

        # Récupérer toutes les maisons (groupes)
        maisons = Maison.objects.all()

        if receiver_id:
            # Si un utilisateur spécifique est sélectionné, récupérer les messages individuels
            receiver = get_object_or_404(CustomUser, id=receiver_id)
            messages = Message.objects.filter(
                Q(sender=user, receiver=receiver) |
                Q(sender=receiver, receiver=user)
            ...
            # Sinon, récupérer tous les messages liés à l'utilisateur (individuels et de groupe)
            messages = Message.objects.filter(
                Q(sender=user) |
                Q(receiver=user) |
        ...
        context['all_users'] = all_users
        context['maisons'] = maisons  # Ajouter les maisons au contexte
        return context
############################################################################################################################################################""""
