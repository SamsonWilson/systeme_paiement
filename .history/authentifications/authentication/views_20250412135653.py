...
#     model = PaiementLoyer
#     form_class = PaiementLoyerForm
#     template_name = 'accuiel/paiements/paiement_form.html'  # Standard path
#     success_url = reverse_lazy("liste_paiements")  # Use namespace if defined

#     def get_location_object(self):
#         """Helper method to retrieve the Location object."""
#         location_id = self.kwargs.get('location_id')
#         if not location_id:
#             messages.error(self.request, "No location specified.")
#             return None
#         try:
#             # Use an attribute to avoid multiple lookups if needed
#             if not hasattr(self, '_location_obj'):
#                 self._location_obj = get_object_or_404(Location, id=location_id)
#             return self._location_obj
#         except Location.DoesNotExist:
#             messages.error(self.request, "The specified location does not exist.")
#             return None
...
#             kwargs['location'] = location
#         else:
#             # If the location is not found, redirect to prevent further errors
#             messages.error(self.request, "Cannot create payment: Location not found.")
#             # Use reverse() here since we need the URL immediately
#             redirect_url = reverse('liste_locations') # Replace it for real list of locations
...
#         if location:
#             context['location'] = location
#              # Template JS requires these values
#             context['montant_mensuel'] = location.montant_total  # Pass the amount used for calculation
#             context['date_fin_avance_iso'] = location.date_fin_avance.isoformat() if location.date_fin_avance else None
#         return context
...
#         """Called if the form is valid. Handles saving and redirection."""
#         try:
#             # form.save() will now handle the complex logic (commission, dates, pdf, email)
#             self.object = form.save()  # commit=True by default
#             messages.success(
#                 self.request,
#                 f"Payment #{self.object.id} for {self.object.location} successfully registered. "
#                 f"A receipt has been sent to {self.object.location.utilisateur.email}."
#             )
#             # Redirects to success_url defined above
#             return super().form_valid(form)
#         except Exception as e:
#             # Catches potential errors from form.save() (PDF, email, etc.)
#             error_message = f"An error occurred while saving or sending the receipt: {e}"
#             print(f"ERROR in form_valid PaiementLoyerCreateView: {error_message}")  # Detailed log
#             messages.error(self.request, error_message)
#             # Return to the form with the error
#             return self.form_invalid(form)

#     def form_invalid(self, form):
#         messages.error(self.request, "The form contains errors. Please check the fields.")
#         return super().form_invalid(form)

#     def dispatch(self, request, *args, **kwargs):
#         # Check if the location exists before even trying to display the form
#         location = self.get_location_object()
#         if not location:
#             # Redirect to an appropriate page if the location is not found
#             return redirect(reverse_lazy('liste_locations'))  # Example

#         return super().dispatch(request, *args, **kwargs)
    # 
# class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
#     model = PaiementLoyer
#     template_name = 'accuiel/paiements/paiement_form.html'  # Correction de la faute de frappe "accuiel" -> "accueil"
#     form_class = PaiementLoyerForm
#     success_url = reverse_lazy("liste_paiements")
...
#             initial['montant_paye'] = location.chambre.prix if location.chambre else 0  # Utilisez le prix de la chambre
#         return initial
...
#             location = get_object_or_404(Location, id=location_id)
#             context['location'] = location
#             # Ne désactivez pas le champ 'location' dans le formulaire pour permettre la soumission
#             # Utilisez plutôt un champ caché pour transmettre la valeur
#         return context
...
#             location_id = self.kwargs.get('location_id')
#             if not location_id:
#                 raise ValueError("L'ID de la location est manquant.")
            
#             location = get_object_or_404(Location, id=location_id)
#             form.instance.location = location

#             # Calculez le montant payé en fonction du nombre de mois
#             nombre_mois = form.cleaned_data.get('nombre_mois_paye', 0)
#             form.instance.montant_paye = location.chambre.prix * nombre_mois if location.chambre else 0

#             # Sauvegardez le formulaire
#             response = super().form_valid(form)
#             messages.success(self.request, 'Le paiement a été créé avec succès.')
#             return response
#         except Exception as e:
...
#             dernier_paiement = PaiementLoyer.objects.filter(location=location).order_by('-date_paiement').first()
...
#                 date_fin = location.date_fin_avance

#             date_expiree = False
#             if date_fin and date_fin < current_date_togo.date():
#                 date_expiree = True
...
#         # Trier les paiements par `date_fin` en ordre décroissant
#         paiements_par_location.sort(key=lambda x: x['date_fin'] or datetime.min.date(), reverse=True)

#         context['paiements_par_location'] = paiements_par_location
#         context['current_date_togo'] = current_date_togo
#         context['total_paiements'] = PaiementLoyer.objects.count()
#         context['paiements_confirmes'] = PaiementLoyer.objects.filter(statut_paiement="confirmé").count()
#         context['paiements_en_attente'] = PaiementLoyer.objects.filter(statut_paiement="en attente").count()
...
class PaiementLoyerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 🔁 Trier les paiements par `date_fin` en ordre croissant
        paiements_par_location.sort(
            key=lambda x: x['date_fin'] if isinstance(x['date_fin'], date) and x['date_fin'] is not None else date(9999, 12, 31)  # noqa: F821
        )        
