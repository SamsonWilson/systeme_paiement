...
class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
 ...
 model = PaiementLoyer
    ...
    success_url = reverse_lazy("VOTRE_APP:liste_paiements")  # Utilisez le namespace si défini
    ...
    def get_form_kwargs(self):
        ...
        if location:
            ...
            redirect_url = reverse('liste_locations')  # Remplacez par l'URL réelle
            kwargs['redirect_url'] = redirect_url
        return kwargs

    def get_context_data(self, **kwargs):
        """Ajoute la location au contexte pour l'affichage dans le template."""
        context = super().get_context_data(**kwargs)
        location = self.get_location_object()
        if location:
            context['location'] = location
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            print(f"ERROR in form_valid PaiementLoyerCreateView: {e}")
            return self.form_invalid(form)
