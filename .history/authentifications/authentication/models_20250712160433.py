...
class CustomUser(AbstractUser):
    class Meta(AbstractUser.Meta):        # Lister ici TOUS les champs que l'utilisateur doit remplir lors de l'inscription
        fields = (
            'username', 'email', 'nom', 'prenom', 'gender', 'tel', 'image',
            'nom_personne_prevenir', 'prenom_personne_prevenir', 'telephone_personne_prevenir',
            'type_utilisateur' # Assurez-vous que l'utilisateur doit bien choisir son type à l'inscription
        )

    def __init__(self, *args, **kwargs):
        """
        Cette méthode est facultative mais très utile pour rendre le formulaire plus joli
        avec Crispy Forms ou Bootstrap manuellement.
        """
        super().__init__(*args, **kwargs)
        # Rendre les labels plus clairs pour l'utilisateur
        self.fields['email'].required = True
        self.fields['nom'].label = "Nom de famille"
        self.fields['prenom'].label = "Prénom"
        self.fields['tel'].label = "Numéro de téléphone"
        self.fields['image'].label = "Photo de profil (facultatif)"
        self.fields['gender'].label = "Genre"
        self.fields['nom_personne_prevenir'].label = "Nom de la personne à prévenir"
        self.fields['prenom_personne_prevenir'].label = "Prénom de la personne à prévenir"
        self.fields['telephone_personne_prevenir'].label = "Téléphone de la personne à prévenir"
        self.fields['type_utilisateur'].label = "Vous êtes ?"

        # Ajouter des placeholders pour guider l'utilisateur
        self.fields['username'].widget.attrs['placeholder'] = "Choisissez un nom d'utilisateur unique"
        self.fields['email'].widget.attrs['placeholder'] = "votre.email@exemple.com"

        # Améliorer l'affichage du champ 'gender'
        # Au lieu d'une liste déroulante, on utilise des boutons radio
        self.fields['gender'].widget = forms.RadioSelect()
        # Pour que les radios s'affichent correctement avec Bootstrap, on doit les manipuler dans le template
        # ou utiliser Crispy Forms qui s'en charge.

    def save(self, commit=True):
        """
        On surcharge la méthode save pour s'assurer que les champs first_name et last_name
        du modèle User de base sont aussi remplis.
        """
...
# class MessageIndiv(models.Model):
#     sender = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='sent_messages')
#     receiver = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='received_messages', null=True, blank=True)
#     message = models.TextField()
...
class Proprietaire(models.Model):
    ...
    def get_nom_complet(self):
        return f"{self.get_gender_display()} {self.prenom} {self.nom}"
