...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if hasattr(user, 'type_utilisateur') and user.type_utilisateur.nom == 'locataire':
            # Si l'utilisateur est locataire, il ne voit que les admins
            all_users_query = CustomUser.objects.filter(type_utilisateur__nom='admin')
        else:
            # Admins et propriétaires voient tout le monde sauf eux-mêmes
            all_users_query = CustomUser.objects.exclude(id=user.id)

        # --- Ajouter le nombre de messages non lus pour chaque utilisateur ---
        all_users = []
        ...
        # --- Si un receiver est sélectionné ---
        if receiver_id:
            receiver = get_object_or_404(CustomUser, id=receiver_id)

            # Marquer tous ses messages comme lus
            ...
            # Charger les messages échangés avec ce receiver
            messages = Message.objects.filter(
                Q(sender=user, receiver=receiver) |
                Q(sender=receiver, receiver=user)
            ).order_by('timestamp')
            ...
            # Sinon, charger tous les messages où l'utilisateur est impliqué
            messages = Message.objects.filter(
        ...
        context['messages'] = messages
        context['user'] = user
        context['all_users'] = all_users  # <-- Attention : ce n'est plus juste une liste de users
        context['is_admin'] = hasattr(user, 'type_utilisateur') and user.type_utilisateur.nom == 'admin'
