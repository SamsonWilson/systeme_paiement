...
class AdminMessageCreateView(CreateView):
    model = Message
    form_class = AdminMessageForm
    template_name = "messagerie/message_form.html"
    success_url = ...  # adapte selon ton URL
    ...
    def form_valid(self, form):
        form.instance.sender = self.request.user
        form.instance.is_group_message = True  # forcer message groupé pour cette vue spéciale
        return super().form_valid(form)
# dans views.py
# dans views.py
...
#         else:
#             maisons = request.user.get_maisons_louees()

#         conversations = []
#         for maison in maisons:
#             last_message = Message.objects.filter(maison=maison).order_by('-timestamp').first()
#             unread_count = Message.objects.filter(maison=maison, is_read=False).exclude(sender=request.user).count()

#             conversations.append({
#                 'maison': maison,
#                 'last_message': last_message,
#                 'unread_count': unread_count,
#                 'locataires': maison.get_locataires_actifs() if request.user.is_staff else [],
#             })
        
#         # Cette ligne dépend de l'import : from datetime import datetime, timezone
...
class InboxView(ListView):
    ...
    #     for maison in maisons:
    #         last_message = (
    #             Message.objects.filter(maison=maison)
    #             .order_by('-timestamp')
    #             .first()
    #         )
    #         unread_count = (
    #             Message.objects.filter(maison=maison, is_read=False)
    ...
    #     # Valeur par défaut pour les conversations sans message
    #     safe_min_datetime = timezone.make_aware(
    #         datetime(1970, 1, 1),  # <-- évite les erreurs de datetime.min
    #         timezone=timezone.get_current_timezone()
    #     )

    #     conversations.sort(
    #         key=lambda x: x['last_message'].timestamp if x['last_message'] else safe_min_datetime,
    #         reverse=True
    #     )
...
class MaisonMessageIndividueleLocataireView(LoginRequiredMixin, View):
    """
    Affiche la conversation d'une maison et gère l'envoi de messages.
    """
    form_class = AdminMessageForm
...
class MessageListView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_staff:
            # MODIFIÉ : L'admin récupère TOUTES les maisons, pas seulement celles avec des messages.
            maisons = Maison.objects.all().order_by('nom')
        else:
            maisons = request.user.maisons_louees.all()

        conversations = []
        for maison in maisons:
            last_message = Message.objects.filter(maison=maison).order_by('-timestamp').first()
            
            # Pour une maison qui a au moins un message, on calcule le nombre de non-lus
            if last_message:
                unread_count = Message.objects.filter(maison=maison, is_read=False).exclude(sender=request.user).count()
            # Pour une nouvelle conversation, le compteur est à 0
            else:
                unread_count = 0

            # MODIFIÉ : On ajoute TOUJOURS la maison à la liste, même sans message.
            conversations.append({
                'maison': maison,
                'last_message': last_message, # Sera 'None' si aucun message n'existe
                'unread_count': unread_count,
               'locataires': maison.get_locataires_actifs(),
            })
        
        # MODIFIÉ : La clé de tri doit gérer le cas où 'last_message' est None.
        # On trie pour que les conversations sans message apparaissent à la fin.
        conversations.sort(
