...
class ChatView(LoginRequiredMixin, FormView):  # noqa: F821class ChatView(LoginRequiredMixin, FormView):
    def get(self, request, receiver_id):
        ...
        message_data = [{"sender": msg.sender.username, "message": msg.message, "timestamp": msg.timestamp} for msg in messages]
    ...
    def post(self, request, receiver_id):
        ...
        if message_content:
            ...
            Message.objects.create(sender=user, receiver=receiver, message=message_content)  # Utilisez 'message' ici
            
            # Vous pouvez retourner une réponse JSON avec les derniers messages si nécessaire
            return JsonResponse({"message": "Message envoyé avec succès"})
        
        return JsonResponse({"message": "Échec de l'envoi du message"}, status=400)
