...
class PDFTemplateViewLocation(View):
    ...
    def get(self, request, location_id, *args, **kwargs):
        try:
            # 1. Récupérer l'objet Location
            location = get_object_or_404(Location, id=location_id)

            # 2. Préparer le contexte pour le template
            context = {'location': location}

            # 3. Rendre le template en une chaîne de caractères HTML
            #    Utilisez le template professionnel que nous avons créé précédemment.
            html_string = render_to_string('pdf/contrat_location_pro.html', context)
            
            # 4. Construire une URL de base pour que WeasyPrint trouve les fichiers statiques (CSS, images)
            #    C'est une étape CRUCIALE pour que le design fonctionne.
            base_url = request.build_absolute_uri('/')

            # 5. Générer le PDF en mémoire
            #    On passe la chaîne HTML et l'URL de base à WeasyPrint.
            pdf_file = HTML(string=html_string, base_url=base_url).write_pdf()

            # 6. Créer une réponse HTTP avec le contenu du PDF
            response = HttpResponse(pdf_file, content_type='application/pdf')

            # 7. Définir les en-têtes pour le navigateur
            #    'inline' : essaie d'afficher le PDF dans le navigateur.
            ...
            # En cas d'erreur, il est utile de l'afficher pour le débogage
            print(f"Erreur lors de la génération du PDF pour location {location_id}: {e}")
            return HttpResponse(f"Une erreur est survenue lors de la génération du PDF. Détails: {e}", status=500)
