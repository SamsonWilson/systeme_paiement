...
class  ProprietaireDetaimaisonlView(DetailView):
    ...
    template_name = 'accuiel/proprietaires/proprietaireDetail_Maison.html'
     def get_context_data(self, **kwargs):
        ...
        # Toutes les locations (passées et en cours), le plus récent au plus ancien
        locations = chambre.locations.select_related('utilisateur').order_by('-date_debut_location')

        # On prépare la liste des locations enrichies avec informations nécessaires
        locations_infos = []
        for location in locations:
            fin_location = getattr(location, 'fin_location', None)  # FinLocation possible
            periode = {
            ...
            locations_infos.append({
                "utilisateur": location.utilisateur,
                "montant_avance": location.montant_avance,
                "montant_caution": location.montant_caution,
                "taux_commission": chambre.taux_commission,  # taux au moment de l'affichage
                "statut_paiement": location.statut_paiement,
                "mode_paiement": location.mode_paiement,
                "periode": periode,
            })
        
        # Statut actuel de la chambre / info synthétique
        context['etat'] = chambre.etat
        context['surface'] = chambre.surface
        context['prix'] = chambre.prix
        context['taux_commission'] = chambre.taux_commission
        context['locations_infos'] = locations_infos
