...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if receiver_id:
            ...
            # Si l'utilisateur est admin, il peut voir tous les messages
            if user.type_utilisateur.nom == 'admin':
                ...
                # Les locataires ne peuvent envoyer des messages qu'aux admins
                if receiver.type_utilisateur.nom == 'admin':
                    messages = Message.objects.filter(
                        Q(sender=user, receiver=receiver) |
                        Q(sender=receiver, receiver=user)
                    ).order_by('timestamp')
                else:
                    # En cas de locataire à locataire, ce n'est pas autorisé.
                    messages = Message.objects.none()  # Aucun message autorisé
                context['chat_user'] = receiver

            context['maison'] = None  # Pas de maison dans ce cas
