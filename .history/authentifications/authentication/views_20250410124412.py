...
class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy("VOTRE_APP:liste_paiements")  # Use namespace if defined
...
class PDFTemplateView(View):
    ...
    #         template = get_template('recu/recu_Location.html')  # Assurez-vous d'avoir un template pdf_location.html
    #         context = {'location': location}
    #         html = template.render(context)
    #         response = HttpResponse(content_type='application/pdf')
    #         response['Content-Disposition'] = f'attachment; filename="location_{location_id}.pdf"'
    #         pisa_status = pisa.CreatePDF(io.BytesIO(html.encode('UTF-8')), dest=response)
    #         if pisa_status.err:
    #             return HttpResponse('Erreur lors de la génération du fichier PDF <pre>' + html + '</pre>')
    #         return response
def get(self, request, location_id):
        ...
        # Stocker temporairement le PDF dans la session
        request.session['pdf_data'] = pdf_file.getvalue().decode('latin1')  # Encode comme string pour la session

        return redirect('serve_pdf_and_redirect')  # Rediriger vers la vue finale    