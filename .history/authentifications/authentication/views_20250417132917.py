...
class FinLocationListView(ListView):
    """
    Vue pour lister toutes les chambres dont l'état est 'libre'.
    Inclut les informations sur la maison et le propriétaire associés.
    """
    model = Chambre  # Le modèle principal que nous listons
    template_name = 'accuiel/Finlocation/list_utilisateur.html'
    context_object_name = 'chambres_libres'  # Nom de la variable dans le template

    def get_queryset(self):
        """
        Surcharge la méthode pour ne retourner que les chambres libres
        et optimiser la requête pour récupérer les objets liés (Maison, Proprietaire).
        """
        # Filtre les chambres par état 'libre'
        # Utilise select_related pour optimiser les requêtes SQL en récupérant
        # les objets Maison et Proprietaire liés en une seule fois.
        queryset = Chambre.objects.filter(etat='occupée').select_related(
            'maison', 
            'maison__proprietaire' # Suit la relation de Maison vers Proprietaire
            'locations',                # Précharge les locations liées à chaque chambre
            'locations__utilisateur'    # Précharge l'utilisateur lié à chaque location préchargée
        )
        return queryset

    def get_context_data(self, **kwargs):
        """
        Ajoute des informations supplémentaires au contexte si nécessaire.
        (Optionnel dans ce cas simple, mais utile pour des contextes plus complexes).
        """
        context = super().get_context_data(**kwargs)
        # Vous pourriez ajouter d'autres informations ici si besoin
        # context['titre_page'] = "Liste des Chambres Disponibles"
        return context
