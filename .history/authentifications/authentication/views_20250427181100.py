...
class ChatView(LoginRequiredMixin, View):
    def get(self, request, receiver_id):
        user = request.user
        try:
            chat_user = CustomUser.objects.get(id=receiver_id)  # Trouver l'utilisateur avec qui vous discutez
        except CustomUser.DoesNotExist:
            raise Http404("L'utilisateur demandé n'existe pas.")
    
        # Charger les messages entre l'utilisateur connecté et le destinataire
        messages = Message.objects.filter(
            (Q(sender=user) & Q(receiver_id=receiver_id)) |
            (Q(sender_id=receiver_id) & Q(receiver=user))
        ).order_by('timestamp')

        # Passer les messages et l'utilisateur du chat au template
    ...
    def post(self, request, receiver_id):
        ...
        try:
            ...
            return JsonResponse({"message": "Utilisateur destinataire introuvable"}, status=404)

        # Création du message
        Message.objects.create(sender=user, receiver=receiver, message=message_content)

        # Retourner une réponse JSON après l'envoi du message
        return JsonResponse({
            "message": "Message envoyé avec succès", "sender": user.username,
            "receiver": receiver.username,
            "message_content": message_content,
            "timestamp": timezone.now().strftime('%H:%M')  # format du timestam
        })
