...
class PrivateMessageForm(forms.ModelForm):
    """
    Formulaire pour l'envoi de messages privés.
    """
    class Meta:
        model = Message
        ...
        def clean(self):
            """
            Validation personnalisée pour s'assurer que le message n'est pas totalement vide.
            """
            cleaned_data = super().clean()
            message = cleaned_data.get('message')
            image = cleaned_data.get('image')

            # Si ni le texte ni l'image ne sont fournis, on lève une erreur de validation.
            if not message and not image:
                raise forms.ValidationError(
                    "Vous ne pouvez pas envoyer un message vide. Veuillez écrire un texte ou joindre une image."
                )
...
#             # Filtre le queryset comme vous le faites déjà (c'est une bonne pratique)
#             self.fields['utilisateur'].queryset = CustomUser.objects.filter(type_utilisateur__nom='propriétaire')
            
#             # C'est ici que la magie opère !
#             # On assigne une fonction lambda qui formate le texte pour chaque objet.
#             self.fields['utilisateur'].label_from_instance = lambda obj: f"{obj.get_full_name()} ({obj.username})"

# ... (les imports de l'étape 1 sont juste au-dessus) ...

class ProprietaireForm(forms.ModelForm):
    """
    Formulaire pour créer ou modifier un Proprietaire.
    La liste déroulante 'utilisateur' est filtrée pour n'afficher
    que les utilisateurs de type 'propriétaire' et affiche leur nom complet.
    """
    class Meta:
        model = Proprietaire
        fields = ['utilisateur']
        widgets = {
            # Votre template s'occupe déjà de la classe CSS avec widget_tweaks,
            # donc on peut laisser le widget simple ici.
            'utilisateur': forms.Select(),
        }

    def __init__(self, *args, **kwargs):
        # On appelle le constructeur parent pour initialiser les champs
        super().__init__(*args, **kwargs)

        # ---- DÉBUT DE LA LOGIQUE DE CORRECTION ----

        # 1. FILTRAGE : On récupère uniquement les utilisateurs dont le type est 'propriétaire'
        #    La chaîne 'propriétaire' doit correspondre EXACTEMENT à la valeur
        #    stockée dans votre base de données (minuscules, accent, etc.)
        queryset_filtre = User.objects.filter(type_utilisateur__nom='propriétaire')

        # On applique ce filtre au champ 'utilisateur' du formulaire
        self.fields['utilisateur'].queryset = queryset_filtre

        # 2. AFFICHAGE DU NOM : On définit comment chaque utilisateur doit être affiché
        #    On affiche "Prénom Nom (username)". S'il n'y a pas de prénom/nom,
        #    on affiche juste "username".
        self.fields['utilisateur'].label_from_instance = lambda obj: (
        f"{obj.nom} {obj.prenom} (- Type: {obj.type_utilisateur.nom if obj.type_utilisateur else 'Aucun'}) - Email: {obj.email} ")
        # 3. MESSAGE D'AIDE : On améliore l'expérience utilisateur
        #    Si la liste des propriétaires est vide, on affiche un message clair.
        if not queryset_filtre.exists():
            self.fields['utilisateur'].empty_label = "Aucun utilisateur de type 'Propriétaire' n'a été trouvé"
        else:
            self.fields['utilisateur'].empty_label = "--- Sélectionnez un propriétaire ---"
    def clean_utilisateur(self):
            ...
            if Proprietaire.objects.filter(utilisateur=utilisateur).exists():
                raise forms.ValidationError("Ce propriétaire existe déjà.")
            return utilisateur
...
class MaisonForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'nombre_piece': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Nombre de pièce'}),
        }
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Afficher uniquement les propriétaires ayant un utilisateur
        self.fields['proprietaire'].queryset = Proprietaire.objects.filter(utilisateur__isnull=False)
    ...
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(attrs={
             ...
             # On ajoute le widget pour le champ 'piece'
        }
    # def clean(self):
    #     cleaned_data = super().clean()
...
#     """
#     TAUX_COMMISSION_CHOICES = [(i, f"{i}%") for i in range(0, 31)]
    ...
    class Meta:
        ...
        widgets = {
            ...
            'surface': forms.TextInput(attrs={
                'class': 'form-control',
                'readonly': 'readonly',
                'onclick': 'ouvrirSurfaceOptions()',
                'placeholder': 'Cliquez pour calculer'
            }),
            'prix': forms.NumberInput(attrs={'class': 'form-control'}),
            'etat': forms.Select(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control'}),
            'maison': forms.Select(attrs={'class': 'form-control', 'disabled': 'disabled'}),
        }
...
#         model = Chambre
#         exclude = ['etat'] 
#         exclude = ['surface']  # Surface est calculée automatiquement
#         widgets = {
#             'type_chambre': forms.Select(attrs={'class': 'form-control'}),
#             'prix': forms.NumberInput(attrs={'class': 'form-control'}),
#             'etat': forms.Select(attrs={'class': 'form-control'}),
...
#     def __init__(self, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         # Empêcher la modification du champ maison si instance existe
...
#         if maison and maison.nombre_piece <= maison.chambres.count():
#             raise forms.ValidationError(
#                 f"La maison '{maison.nom}' n'a plus de pièces disponibles."
#             )
#         return cleaned_data        
