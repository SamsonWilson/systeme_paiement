...
class FinLocationListView(ListView):
    ...
    def get_queryset(self):
        return Location.objects.select_related('utilisateur', 'chambre', 'chambre__maison').all()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)

        # Récupérer toutes les locations avec infos utilisateur, chambre et maison
        locations_details = []
        for location in self.get_queryset():
            locations_details.append({
                'utilisateur': location.utilisateur,
                'chambre': location.chambre,
                'maison': location.chambre.maison if location.chambre else None,
                'montant_total': location.montant_total,
                'date_debut': location.date_debut_location,
                'date_fin_avance': location.date_fin_avance,
                'paiements': location.paiements.all(),  # reverse FK
            })

        context.update({
            'locations_details': locations_details,
            'total_locations': len(locations_details),
        })

        # Gestion recherche (facultative)
        query = self.request.GET.get('q')
        if query:
            filtered = [
                loc for loc in locations_details
                if query.lower() in loc['utilisateur'].nom.lower() or
                   query.lower() in loc['utilisateur'].prenom.lower()
            ]
            context['locations_details'] = filtered
            context['search_results_count'] = len(filtered)
