...
#     def get_context_data(self, **kwargs):
#         # Récupération du locataire basé sur l'ID passé en paramètre
#         locataire_id = self.kwargs.get('locataire_id')
#         locataire = CustomUser.objects.get(id=locataire_id)

#         # Transactions liées au locataire
#         paiements = PaiementLoyer.objects.filter(location__utilisateur=locataire)
...
class LocataireDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Calcul du montant total dû et du montant payé
        montant_total = 0
        montant_paye = 0
        for location in locations:
            # Ajoute le montant total de la location
            montant_total += location.montant_total
            
            # Calcule le montant payé pour cette location
            montant_paye_location = PaiementLoyer.objects.filter(location=location).aggregate(Sum('montant_paye'))['montant_paye__sum'] or 0
            montant_paye += montant_paye_location
