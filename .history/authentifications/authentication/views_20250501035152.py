...
class LocataireDashboardView(TemplateView):
    template_name = 'accuiel/locataire/locataire_dashboard.html'
     ...
     def get_context_data(self, **kwargs):
        ...
        location = Location.objects.filter(utilisateur=utilisateur).first()
        
        # Récupérer les paiements en cours
        paiements = PaiementLoyer.objects.filter(location=location, statut='en cours')
        
        # Calculer le montant total et dû
        montant_total = location.montant_total if location else 0
        montant_paye = sum(p.paiement for p in paiements)
