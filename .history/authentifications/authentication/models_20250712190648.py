...
class Proprietaire(models.Model):
    ...
    utilisateur = models.ForeignKey(
        ...
        if self.utilisateur:
            # Affiche le nom complet, ou le username si le nom n'est pas défini
            nom_a_afficher = self.utilisateur.get_full_name() or self.utilisateur.username
            return f"Propriétaire : {nom_a_afficher}"
        return f"Propriétaire ID {self.id} (non associé à un utilisateur)"
    # nom = models.CharField(max_length=100)
    # prenom = models.CharField(max_length=100)
