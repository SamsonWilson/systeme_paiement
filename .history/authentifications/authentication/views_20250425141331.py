...
from .models import Message  # Import des modèles nécessaires
...
class SendMessageView(LoginRequiredMixin, View):
    template_name = 'accuiel/messages/send_message.html'
    form_class = MessageForm
    success_url = reverse_lazy('chat:boite_de_reception')  # Redirige après succès
    def get_form_kwargs(self):
        """
        Passe l'utilisateur connecté comme expéditeur.
        """
    ...
    def form_valid(self, form):
        """
        Vérifie les permissions et enregistre le message.
        """
        sender = self.request.user
        receiver = form.cleaned_data['receiver']

        # Vérifie si l'expéditeur peut envoyer au destinataire
        if not sender.peut_envoyer_a(receiver):
            messages.error(self.request, "Vous n'avez pas la permission d'envoyer un message à cet utilisateur.")
...
class ChatView(LoginRequiredMixin, TemplateView):
    template_name = 'accuiel/message/message.html'
