...
class FinLocationListView(ListView):
    ...
    def get_queryset(self):
        """
        Retourne les locations où la chambre associée est marquée 'occupée',
        ou une seule chambre si un id est fourni.
        """
        chambre_id = self.kwargs.get("chambre_id")  # Récupère l'ID depuis l'URL s'il existe
        ...
        if chambre_id:  # Si un ID est donné, on filtre plus précisément
    ...
    def get_context_data(self, **kwargs):
        ...
        stats_data = queryset.aggregate(
            total_rooms=Count('chambre', distinct=True),
            total_rent=Sum('chambre__prix'),
            average_rent=Avg('chambre__prix'),
            total_surface=Sum('chambre__surface'),
            average_surface=Avg('chambre__surface')
        )
