...
class MaisonLocatairesView(LoginRequiredMixin, View):
    ...
    def get(self, request, maison_id):
        ...
        if request.user.is_staff or request.user.type_utilisateur.nom == "admin":
            # ✅ Cas ADMIN : voit tous les locataires de la maison
            ...
            context = {
                ...
                'mode': 'admin'   # ➝ pour différencier dans le template
            }

        elif request.user.type_utilisateur.nom == "locataire":
            # ✅ Cas LOCATAIRE : ne voit que l'admin
            admin = CustomUser.objects.filter(type_utilisateur__nom="admin").first()
            if not admin:
                raise PermissionDenied("Aucun administrateur trouvé.")
            context = {
                'maison': maison,
                'admin': admin,
                'mode': 'locataire'
