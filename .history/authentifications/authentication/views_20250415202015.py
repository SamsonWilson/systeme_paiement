...
class PaiementLoyer2CreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        """Called if the form is valid. Handles saving and redirection."""
        try:
            # Récupérer l'objet sans le sauvegarder immédiatement
            self.object = form.save(commit=False)

            # Calculer la date_fin_paiement
            if self.object.nombre_mois_paye and self.object.date_paiement:
                from datetime import timedelta
                self.object.date_fin_paiement = self.object.date_paiement + timedelta(days=30 * self.object.nombre_mois_paye)
...
/*************  ✨ Windsurf Command 🌟  *************/
    def get_success_url(self):
        """
        Constructs the URL to redirect to the PDF generation view after a successful form submission.

        The URL is dynamically generated using the 'generate_pdfpayement' route and includes
        the ID of the 'PaiementLoyer' object as a keyword argument.

        Returns:
            str: A URL string to redirect to the PDF generation view.
        """
        # Use the 'reverse' function to construct the URL with the necessary parameters
        """Redirect to the PDF generation view after successful form submission."""
        return reverse('generate_pdfpayement', kwargs={'paiementloyer_id': self.object.id})
/*******  03154ba4-3e58-4330-98c0-e914dc2e0059  *******/
