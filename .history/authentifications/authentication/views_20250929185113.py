...
class PDFTemplateViewLocation(View):
    def get(self, request, pk, *args, **kwargs):
        try:
            # Utilisez le bon modèle ici (Paiement, Location, etc.)
            
            paiement = get_object_or_404(Paiement, pk=pk)

            context = {
                'paiement': paiement,
                # Ou 'location': location selon le cas
            }

            # Utilisez le bon template
            html_string = render_to_string('pdf/recu_paiement_pro.html', context)
            
            # WeasyPrint a besoin d'une URL de base pour trouver les fichiers statiques (CSS, images)
            ...
            filename = f"recu_{paiement.id}.pdf"
            response['Content-Disposition'] = f'inline; filename="{filename}"'
            
            return response

        except Exception as e:
            # C'est une bonne pratique de logger l'erreur pour le débogage
            print(f"Erreur lors de la génération du PDF : {e}")
