...
class ChatView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        receiver_id = self.kwargs.get('receiver_id')  # Récupérer l'ID du destinataire depuis l'URL
        chat_user = get_object_or_404(CustomUser, id=receiver_id)
        user = self.request.user

        # Récupérer les messages échangés entre l'utilisateur connecté et le destinataire
        messages = Message.objects.filter(
            models.Q(sender=user, receiver=chat_user) |
            models.Q(sender=chat_user, receiver=user)
        ).order_by('timestamp')
