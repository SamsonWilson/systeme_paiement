...
from .models import Proprietaire  # Assurez-vous que le chemin d'importation est correct

class ProprietaireDetailView(DetailView):
    model = Proprietaire
    template_name = 'proprietaire/proprietaire_detail.html'  # Créez ce template

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        proprietaire = self.get_object() # Récupère l'instance de Proprietaire

        # Ajoute les statistiques au contexte
        context['maisons_count'] = proprietaire.maisons_count()
        context['chambres_count'] = proprietaire.chambres_count()
        context['total_commissions_attendues'] = proprietaire.total_commissions_attendues()
        context['total_commissions_perçues'] = proprietaire.total_commissions_perçues()
        context['total_loyers_attendus'] = proprietaire.total_loyers_attendus()
        context['loyers_mensuels_moyens'] = proprietaire.loyers_mensuels_moyens()
        context['chambres_libres_count'] = proprietaire.chambres_libres_count()
        context['chambres_occupees_count'] = proprietaire.chambres_occupees_count()
        context['taux_occupation'] = proprietaire.taux_occupation()
        context['derniers_paiements_loyer'] = proprietaire.derniers_paiements_loyer()
        context['total_montant_avance_locations'] = proprietaire.total_montant_avance_locations()
        context['total_montant_caution_locations'] = proprietaire.total_montant_caution_locations()
        context['locations_en_cours_count'] = proprietaire.locations_en_cours_count()
        context['locations_terminees_count'] = proprietaire.locations_terminees_count()
        context['revenus_totaux'] = proprietaire.revenus_totaux()
        return context

class ProprietaireListView(ListView):
    model = Proprietaire
    template_name = 'proprietaire/proprietaire_list.html' # Créez ce template
