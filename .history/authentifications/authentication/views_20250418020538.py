...
class ListePaiementsLocationView(LoginRequiredMixin, ListView): # Ajout de LoginRequiredMixin (optionnel)
    ...
    def get_context_data(self, **kwargs):
        ...
        paiements_qs = PaiementLoyer.objects.filter(location=location_obj)
        context['total_paid_amount'] = paiements_qs.aggregate(total=Sum('montant_paye'))['total'] or Decimal('0.0')

        # Sommes par statut (adaptés à vos statuts exacts, ajustez les noms si besoin)
        context['paiements_statuts'] = {
            'confirmé': paiements_qs.filter(statut_paiement='confirmé').aggregate(total=Sum('montant_paye'))['total'] or Decimal('0.0'),
            'en attente': paiements_qs.filter(statut_paiement='en attente').aggregate(total=Sum('montant_paye'))['total'] or Decimal('0.0'),
            'rejeté': paiements_qs.filter(statut_paiement='rejeté').aggregate(total=Sum('montant_paye'))['total'] or Decimal('0.0'),
        }
        return context
