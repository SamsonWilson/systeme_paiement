...
class FinLocationListView(ListView):
    model = Location
    template_name = 'accuiel/finlocation/list.html'
    context_object_name = 'locations'

    def get_queryset(self):
        maison_id = self.kwargs.get('maison_id')
        return Location.objects.filter(chambre__maison__id=maison_id).select_related('utilisateur', 'chambre', 'chambre__maison')
