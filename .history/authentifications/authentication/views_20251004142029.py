...
from .forms import  ChambreForm, CustomUserChangeForm, CustomUserCreationForm, CustomAuthenticationForm, CustomUserReinitialisationPasswordForm, LocationForm, MaisonForm, MessageForm, PaiementLoyerForm, PrivateMessageForm, ProprietaireForm, TypeChambreForm, VilleForm, VilleSearchForm,QuartierForm
...
# TOUT EN HAUT DE VOTRE FICHIER views.py

# Import de la bibliothèque standard de Python
from datetime import datetime, timezone  # <--- VÉRIFIEZ BIEN CETTE LIGNE
...
from django.contrib.auth.mixins import LoginRequiredMixin
from django.core.exceptions import PermissionDenied
from .models import Maison, Message, CustomUser, FinLocation # Assurez-vous d'importer tous les modèles nécessaires
from datetime import datetime, timezone
from django.contrib.auth.mixins import LoginRequiredMixin
from django.shortcuts import render
from django.views import View
from .models import Maison, Message
...
# ... autres imports de modèles ...

# -----------------------------------------------------------------
# LE RESTE DE VOTRE FICHIER (VOS VUES) COMMENCE ICI
# -----------------------------------------------------------------
...
from .models import Maison, Message
# c'est pour afficher la liste des maisons dans la boite de reception

class InboxView(ListView):
    model = Maison
    template_name = 'accuiel/messages/inbox.html'
    context_object_name = 'maisons'
    def get_queryset(self):
        user = self.request.user

        if user.type_utilisateur and user.type_utilisateur.nom == "locataire":
            # ✅ Maisons liées aux locations actives du locataire
            return Maison.objects.filter(
                chambres__locations__utilisateur=user
            ).distinct()

        elif user.type_utilisateur and user.type_utilisateur.nom == "propriétaire":
            # ✅ Maisons possédées par le propriétaire
            return Maison.objects.filter(proprietaire__utilisateur=user)

        elif user.type_utilisateur and user.type_utilisateur.nom == "admin":
            # ✅ Admin voit toutes les maisons
            return Maison.objects.all()

        # Par défaut → aucune maison
        return Maison.objects.none()
    def get_context_data(self, **kwargs):
            context = super().get_context_data(**kwargs)
            user = self.request.user

            # Dictionnaire maison_id → nombre de messages non lus
            notifications = {}
            for maison in context["maisons"]:
                unread_count = Message.objects.filter(
                    receiver=user,
                    sender__locations__chambre__maison=maison,
                    is_read=False
                ).count()
                notifications[maison.id] = unread_count

            context["notifications"] = notifications
            return context 
# c'est la conversation de groupe
class MaisonMessageView(LoginRequiredMixin, View):
    """
    Affiche la conversation d'une maison et gère l'envoi de messages.
    """
    ...
    def get(self, request, pk):
        """
        Gère les requêtes GET : Affiche les messages existants et le formulaire.
        """
        maison = get_object_or_404(Maison, pk=pk)
        
        # Récupérer les messages de groupe pour cette maison, triés par date
        messages = Message.objects.filter(
            maison=maison, 
            is_group_message=True
        ).order_by('timestamp')
        ...
        form = self.form_class(user=request.user, initial={
            'maison': maison,
            'is_group_message': True
        })
        ...
        context = {
            'maison': maison,
            'messages': messages,
            'form': form # On passe le formulaire au template (même s'il est personnalisé)
        }
        return render(request, self.template_name, context)

    def post(self, request, pk):
        """
        Gère les requêtes POST : Traite l'envoi d'un nouveau message.
        """
        maison = get_object_or_404(Maison, pk=pk)
        
        # Le formulaire HTML est simple, donc on récupère les données manuellement
        # dans une instance de notre form_class pour validation et traitement.
        form = self.form_class(request.POST, request.FILES, user=request.user)

        # Note: Votre template envoie 'message' et 'image', mais pas les autres champs.
        # Nous allons les définir manuellement ci-dessous.

        message_content = request.POST.get('message', '').strip()
        image_file = request.FILES.get('image')

        # Vérifier si au moins un des deux (message ou image) est présent
        if not message_content and not image_file:
            # On pourrait ajouter un message d'erreur ici si on voulait
            return redirect('maison_messages', pk=maison.pk)

        # Crée une instance de message mais ne la sauvegarde pas encore en base
        new_message = Message(
            sender=request.user,
            maison=maison,
        ...
        new_message.save() # Sauvegarde l'objet pour obtenir un ID

        # Ajoute les locataires actifs de la maison comme destinataires
        locataires = maison.get_locataires_actifs()
        new_message.group_receivers.set(locataires)

        return redirect('maison_messages', pk=maison.pk)

    def get_success_url(self):
        # Cette méthode n'est plus directement utilisée avec la classe View, 
        # mais la logique est dans le redirect() de la méthode post.
        return reverse_lazy('maison_messages', kwargs={'pk': self.kwargs['pk']})

# c'est pour afficher la liste des locataires d'une maison
class MaisonLocatairesView(LoginRequiredMixin, View):
   
    """
    Affiche la liste des locataires d'une maison avec le nombre de messages non lus.
    Optimisée pour minimiser les requêtes à la base de données.
    """
    template_name = 'accuiel/messages/maison_locataires.html'

    # def get(self, request, maison_id):
    #     # Utiliser self.kwargs est plus courant dans les CBV
    ...
    #     if user.is_staff or user.type_utilisateur.nom == "admin":
    #         context["mode"] = "admin"
            
    #         # 1. Première requête : obtenir tous les locataires distincts de la maison
    #         locataires = CustomUser.objects.filter(
    #             type_utilisateur__nom="locataire",
    #             locations__chambre__maison=maison
    #         ).distinct()

    #         # 2. Deuxième requête (optimisée) : obtenir les comptes de tous les messages non lus
    #         # venant de ces locataires, groupés par expéditeur.
    #         unread_counts = Message.objects.filter(
    #             receiver=user,
    #             sender__in=locataires,
    #             is_read=False
    #         ).values('sender_id').annotate(count=Count('id'))
            
    #         # 3. Transformer la liste de dictionnaires en un seul dictionnaire {sender_id: count}
    #         # C'est beaucoup plus pratique pour la suite.
    #         notifications_dict = {item['sender_id']: item['count'] for item in unread_counts}

    #         # 4. Attacher le nombre de messages non lus à chaque objet locataire (très rapide, en mémoire)
    #         for locataire in locataires:
    #             locataire.unread_count = notifications_dict.get(locataire.id, 0)
            
    #         context["locataires"] = locataires

    #     # --- Cas du locataire ---
    #     elif user.type_utilisateur.nom == "locataire":
    #         context["mode"] = "locataire"
            
    #         admin = CustomUser.objects.filter(type_utilisateur__nom="admin").first()
    #         if admin:
    #             unread_count = Message.objects.filter(receiver=user, sender=admin, is_read=False).count()
    #             context["admin"] = admin
    ...
    #     else:
    #         raise PermissionDenied("Vous n'êtes pas autorisé à voir cette page.")
    ...
    def get(self, request, maison_id):
        ...
        # --- Cas de l'administrateur ---
        if user.is_staff or getattr(user.type_utilisateur, 'nom', None) == "admin":
            context["mode"] = "admin"
            
            # Étape 1 : Récupérer tous les locataires de la maison en une seule requête.
            locataires = CustomUser.objects.filter(
                type_utilisateur__nom="locataire",
                locations__chambre__maison=maison
            ).distinct()

            # Étape 2 : Récupérer les comptes de TOUS les messages non lus en une seule requête.
            unread_counts = Message.objects.filter(
                receiver=user,
                sender__in=locataires, # Filtre sur les locataires déjà récupérés
                is_read=False
            ).values('sender_id').annotate(count_unread=Count('id'))
            
            # Étape 3 : Transformer les résultats en un dictionnaire {id_locataire: compte} pour un accès instantané.
            notifications_dict = {
                item['sender_id']: item['count_unread'] for item in unread_counts
            }

            # Étape 4 : Attacher le compte à chaque objet locataire (en mémoire, très rapide).
            # C'est ici qu'on crée l'attribut `unread_count` que le template utilisera.
            for loc in locataires:
                loc.unread_count = notifications_dict.get(loc.id, 0)
            
            context["locataires"] = locataires

        # --- Cas du locataire ---
        elif getattr(user.type_utilisateur, 'nom', None) == "locataire":
            context["mode"] = "locataire"
            
            # Trouve le premier admin disponible pour la conversation.
            admin = CustomUser.objects.filter(type_utilisateur__nom="admin").first()
            if admin:
                # Requête simple, car il n'y a qu'un seul compte à vérifier.
                unread_count = Message.objects.filter(
                    receiver=user, sender=admin, is_read=False
                ).count()
                context["admin"] = admin
                context["unread_count"] = unread_count
            else:
...
# class CheckUnreadMessagesAPIView(LoginRequiredMixin, View):
#     """
#     Vue API qui renvoie des données JSON sur les messages non lus
#     pour une maison donnée. Accessible uniquement via des requêtes GET.
#     """
#     def get(self, request, *args, **kwargs):
#         # 1. Vérification des permissions
        
#         if not (request.user.is_staff or request.user.type_utilisateur.nom == "admin"):
#             return JsonResponse({'error': 'Permission non accordée'}, status=403)

#         # 2. Récupération de l'ID de la maison depuis les arguments de l'URL
#         # Dans les CBV, les paramètres d'URL sont dans `self.kwargs`
#         maison_id = self.kwargs.get('maison_id')
#         if not maison_id:
#             return JsonResponse({'error': 'ID de la maison manquant'}, status=400)

#         # 3. Requête optimisée pour compter les messages non lus par expéditeur
#         unread_counts = Message.objects.filter(
#             receiver=request.user,
#             is_read=False,
#             sender__locations__chambre__maison_id=maison_id
#         ).values(
#             'sender_id'  # Grouper par l'ID de l'expéditeur
#         ).annotate(
#             count=Count('id')  # Compter les messages dans chaque groupe
#         )

#         # 4. Renvoyer les données au format JSON
#         return JsonResponse({'unread_counts': list(unread_counts)})

class CheckUnreadMessagesAPIView(LoginRequiredMixin, View):
    """
    Vue API qui renvoie les comptes de messages non lus pour une maison donnée.
    
    - Requiert une connexion (LoginRequiredMixin).
    - Accessible uniquement par les administrateurs.
    - Renvoie un dictionnaire JSON simple au format : 
      { "id_locataire_1": count_1, "id_locataire_2": count_2, ... }
      Exemple : { "15": 2, "23": 5 }
    """
    def get(self, request, *args, **kwargs):
        # 1. Vérification des permissions : S'assurer que l'utilisateur est un admin
        # Cette partie est déjà correcte.
        if not (request.user.is_staff or getattr(request.user.type_utilisateur, 'nom', None) == "admin"):
            return JsonResponse({'error': 'Permission non accordée'}, status=403)

        # 2. Récupération de l'ID de la maison depuis l'URL
        # Cette partie est déjà correcte.
        maison_id = self.kwargs.get('maison_id')
        if not maison_id:
            return JsonResponse({'error': 'ID de la maison manquant'}, status=400)
        ...
        # 3. Requête optimisée pour compter les messages non lus par expéditeur
        # La requête est déjà très bien optimisée.
        unread_counts = Message.objects.filter(
            receiver=request.user,
            is_read=False,
            sender__locations__chambre__maison_id=maison_id
        ...
        # À ce stade, unread_counts est une liste de dictionnaires :
        # [{'sender_id': 15, 'count': 2}, {'sender_id': 23, 'count': 5}]

        # --- LA CORRECTION EST ICI ---
        # 4. Transformer la liste en un dictionnaire simple, comme attendu par le JavaScript
        unread_map = {
            item['sender_id']: item['count'] for item in unread_counts
        }
        # Maintenant, unread_map est : {15: 2, 23: 5}

        # 5. Renvoyer ce dictionnaire simple directement
        return JsonResponse(unread_map)
...
# c'est pour les message individuels
# class InboxViewlocataire(LoginRequiredMixin, TemplateView):
#     template_name = 'accuiel/messages/inbox.html'
...
#         # Récupérer tous les utilisateurs avec qui une conversation privée a eu lieu
#         # 1. Messages envoyés par l'utilisateur
#         sent_messages_receivers = Message.objects.filter(
#             sender=user, is_group_message=False
#         ).values_list('receiver', flat=True)

#         # 2. Messages reçus par l'utilisateur
#         received_messages_senders = Message.objects.filter(
#             receiver=user, is_group_message=False
#         ).values_list('sender', flat=True)

#         # 3. Combiner et dédoublonner les IDs des interlocuteurs
#         interlocutor_ids = set(sent_messages_receivers) | set(received_messages_senders)
        
#         # Récupérer les objets CustomUser correspondants
#         threads = CustomUser.objects.filter(id__in=interlocutor_ids)

#         context['threads'] = threads
#         return context

# Vue 2: La conversation privée
class PrivateChatView(LoginRequiredMixin, View):
    template_name = 'accuiel/messages/private_chat.html'
    form_class = PrivateMessageForm
    def get(self, request, user_id):
        other_user = get_object_or_404(CustomUser, id=user_id)
        
        # Utilisation de Q objects pour trouver les messages entre les deux utilisateurs
        messages = Message.objects.filter(
            (Q(sender=request.user, receiver=other_user) | Q(sender=other_user, receiver=request.user))
            & Q(is_group_message=False)
        ).order_by('timestamp')
    ...
    def post(self, request, user_id):
        ...
        if form.is_valid():
            ...
            new_message.is_group_message = False # Très important !
            new_message.save()
...
class PaiementLoyerCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse('download_and_redirect_payement1', kwargs={'paiementloyer_id': self.object.id})
...
class PaiementLoyer2CreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.location:
            ...
            return redirect(reverse_lazy('download_and_redirect_payement1'))
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_success_url(self):
        ...
        return reverse('download_and_redirect_payement2', kwargs={'paiementloyer_id': self.object.id})
...
class FinLocationCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return context
    def get_success_url(self):
        return reverse('utilisateurs_parchambre')
# class FinLocationCreateView(CreateView):
#     model = FinLocation
#     form_class = FinLocationForm
#     template_name = 'accuiel/Finlocation/fin_location_form.html'
...
class PaiementLoyerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Utilisation correcte du fuseau horaire avec l'import de pytz
        current_date_togo = timezone.now().astimezone(pytz.timezone('Africa/Lome'))
        chambre_id = self.kwargs.get('chambre_id', None)

        paiements_par_location = []

        # <<< VOTRE CORRECTION EST BONNE ICI >>>
        # On filtre pour ne garder que les locations qui n'ont AUCUNE FinLocation associée.
        # Cela signifie que ce sont les locations ACTIVES.
        base_locations = Location.objects.filter(
            fins_location__isnull=True
        ...
        for location in locations:
            ...
            if dernier_paiement:
                ...
                # Optionnel: Gérer les locations actives sans aucun paiement enregistré
                # On peut les marquer comme expirées par défaut pour attirer l'attention
                paiements_par_location.append({
                    'location': location,
                    'chambre': location.chambre,
                    'locataire': location.utilisateur,
                    ...
                    'date_fin': None,
                    'date_expiree': True  # Ou un autre statut comme 'jamais_paye'
                })

        # Trier par date de fin de paiement (les locations sans paiement seront à la fin)
        paiements_par_location.sort(key=lambda x: x['date_fin'] or (current_date_togo + timezone.timedelta(days=9999)).date())
        ...
        if not chambre_id:
            ...
            # Pas de pagination si on filtre sur une chambre
            page_obj = paiements_par_location
        ...
        context.update({
            ...
            'is_paginated': not chambre_id and hasattr(page_obj, 'has_other_pages') and page_obj.has_other_pages(),
            'current_date_togo': current_date_togo,
            'total_paiements_historique': PaiementLoyer.objects.count(), # Total historique
            'paiements_actifs_regles': sum(1 for p in paiements_par_location if not p['date_expiree']),
            'paiements_actifs_expires': sum(1 for p in paiements_par_location if p['date_expiree']),
            'chambre_affichee': chambre_id,
        })
        return context
...
class Download_location_RedirectView(View):
    """
...
#################################################### pdf pour le premier payement ###################################
class PDFTemplateViewpaiyement1(View):
    def get(self, request, paiementloyer_id, *args, **kwargs):
        try:
            paiement = get_object_or_404(PaiementLoyer, id=paiementloyer_id)
            # Contexte envoyé au template
            context = {
                        'paiement': paiement,
                        'location': paiement.location,  # la location liée
                        'chambre': paiement.location.chambre if paiement.location else None,
                    }

            # On rend le template du reçu
            html_string = render_to_string('Recu/Recu_payement_debut.html', context)
            base_url = request.build_absolute_uri('/')
            pdf_file = HTML(string=html_string, base_url=base_url).write_pdf()
            ...
            print(f"Erreur lors de la génération du PDF pour paiement {paiementloyer_id}: {e}")
            return HttpResponse(f"Erreur lors de la génération du PDF: {e}", status=500)


class Download_Payement1_RedirectView(View):
    """
    Cette vue sert une page HTML qui déclenche le téléchargement du PDF 
    puis redirige l'utilisateur vers la page d'accueil.
    """
    def get(self, request, paiementloyer_id, *args, **kwargs):
        context = {
            'pdf_url': reverse('generate_pdfpayement1', args=[paiementloyer_id]),
            'home_url': reverse('liste_paiements')  # redirection après download
        }
        return render(request, 'messageDeSucce/succe_Payement1.html', context)

#################################################### pdf pour le deuxieme payement ###################################
class PDFTemplateViewpaiyement2(View):
    def get(self, request, paiementloyer_id, *args, **kwargs):
        try:
            paiement = get_object_or_404(PaiementLoyer, id=paiementloyer_id)
            # Contexte envoyé au template
            ...
            # On rend le template du reçu
            html_string = render_to_string('Recu/Recu_payement.html', context)
            base_url = request.build_absolute_uri('/')
            pdf_file = HTML(string=html_string, base_url=base_url).write_pdf()
...
class Download_Payement2_RedirectView(View):
    """
    Cette vue sert une page HTML qui déclenche le téléchargement du PDF 
    puis redirige l'utilisateur vers la page d'accueil.
    """
    def get(self, request, paiementloyer_id, *args, **kwargs):
        context = {
            'pdf_url': reverse('generate_pdfpayement2', args=[paiementloyer_id]),
            'home_url': reverse('liste_paiements')  # redirection après download
        }
        return render(request, 'messageDeSucce/succe_Payement2.html', context)
