...
class InboxView(ListView):
    ...
    # 2. Spécifiez le nom de la variable dans le template (ce sera 'conversations')
    context_object_name = 'conversations'

    def get_queryset(self):
        """
        Cette méthode va construire la liste de "conversations" complexe.
        Au lieu de retourner juste Maison.objects.all(), nous allons construire
        une liste de dictionnaires, où chaque dictionnaire représente une conversation.
        """
        user = self.request.user
        
        # Récupérez les maisons pertinentes (par exemple, toutes si vous êtes admin)
        maisons = Maison.objects.all() 
        
        conversations_list = []
        for maison in maisons:
            # Pour chaque maison, on calcule les données supplémentaires
            
            # a) Trouver le dernier message de la conversation
            last_message = Message.objects.filter(maison=maison).order_by('-timestamp').first()
            
            # b) Compter les messages non lus pour l'utilisateur actuel
            #    (Adaptez cette logique si votre modèle de "lu/non lu" est différent)
            unread_count = Message.objects.filter(
                maison=maison, 
                is_read=False
            ).exclude(sender=user).count()

            # c) Récupérer les locataires associés à la maison
            locataires = maison.locataires.all() # Assumant une relation 'locataires'

            # d) On assemble tout dans un dictionnaire
...
class MaisonMessageView(LoginRequiredMixin, CreateView):
    """Affiche et gère l'envoi de messages dans une conversation."""
    model = Message
    form_class = AdminMessageForm
    template_name = 'accuiel/messages/maison_messages.html'

    def get_initial(self):
        maison = get_object_or_404(Maison, pk=self.kwargs['pk'])
        return {'maison': maison, 'is_group_message': True}
    ...
    def form_valid(self, form):
        maison = get_object_or_404(Maison, pk=self.kwargs['pk'])
        form.instance.sender = self.request.user
        form.instance.maison = maison
        form.instance.is_group_message = True
        # Ajout des locataires actifs de la maison comme destinataires
        form.save()  # Il faut sauvegarder pour pouvoir utiliser M2M
        locataires = maison.get_locataires_actifs()
        form.instance.group_receivers.set(locataires)
        return super().form_valid(form)

    def get_success_url(self):
        return reverse_lazy('maison_list')
