...
class MaisonMessageView(LoginRequiredMixin, View):
    template_name = 'accuiel/messages/maison_chat.html' # Nouveau template

    def get(self, request, maison_id):
        maison = get_object_or_404(Maison, pk=maison_id)

        # Sécurité : Vérifier que l'utilisateur a le droit de voir ces messages
        if not request.user.is_staff and maison not in request.user.maisons_louees.all():
            raise PermissionDenied("Vous n'avez pas accès à cette conversation.")

        # Récupérer les messages et marquer comme lus
        messages = Message.objects.filter(maison=maison)
        messages.exclude(sender=request.user).update(is_read=True)
        
        form = MessageForm()
        return render(request, self.template_name, {'maison': maison, 'messages': messages, 'form': form})

    def post(self, request, maison_id):
        maison = get_object_or_404(Maison, pk=maison_id)
        
        # Sécurité (aussi nécessaire dans le POST)
        if not request.user.is_staff and maison not in request.user.maisons_louees.all():
        ...
        if form.is_valid():
            ...
            new_message.maison = maison
            new_message.save()
            return redirect('maison_messages', maison_id=maison_id)
...
class MessageListView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        # ---- DEBUT DEBUG ----
        print("--- Début du traitement de InboxView ---")
        print(f"Utilisateur connecté: {request.user.username}, Staff: {request.user.is_staff}")
        # ---- FIN DEBUG ----

        if request.user.is_staff:
            maisons = Maison.objects.filter(messages__isnull=False).distinct()
        else:
            # Assurez-vous que la relation s'appelle bien 'maisons_louees' sur votre CustomUser
            maisons = request.user.maisons_louees.all() 

        # ---- DEBUT DEBUG ----
        print(f"Queryset de maisons initial: {maisons}")
        print(f"Nombre de maisons trouvées: {maisons.count()}")
        # ---- FIN DEBUG ----

        conversations = []
        for maison in maisons:
            # ---- DEBUT DEBUG ----
            print(f"\n-> Analyse de la maison : '{maison.nom}' (ID: {maison.id})")
            # ---- FIN DEBUG ----
            
            last_message = Message.objects.filter(maison=maison).order_by('-timestamp').first()
            
            # ---- DEBUT DEBUG ----
            print(f"   Dernier message trouvé pour cette maison: {last_message}")
            # ---- FIN DEBUG ----
            ...
            if last_message:
                ...
                # ---- DEBUT DEBUG ----
                print(f"   Nombre de messages non lus: {unread_count}")
                print("   => Cette maison sera ajoutée à la liste des conversations.")
                # ---- FIN DEBUG ----
                ...
                conversations.append({
                    ...
                    'locataires': maison.locataires.all(),
                })
            else:
                 # ---- DEBUT DEBUG ----
                print("   => PAS de dernier message. Cette maison est ignorée.")
                # ---- FIN DEBUG ----
        
        # ---- DEBUT DEBUG ----
        print(f"\nListe 'conversations' finale envoyée au template: {conversations}")
