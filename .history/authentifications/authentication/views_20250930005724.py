...
class PDFTemplateViewpaiyement1(View):
   ...
   def get(self, request, location_id, *args, **kwargs):
       try:
        paiementLoyer = get_object_or_404(PaiementLoyer, id=PaiementLoyer_id)
        context = {'location': location}
        html_string = render_to_string('Recu/Recu_payement_debut.html', context) # Assurez-vous que le chemin est correct
        base_url = request.build_absolute_uri('/')
        pdf_file = HTML(string=html_string, base_url=base_url).write_pdf()
        response = HttpResponse(pdf_file, content_type='application/pdf')
        # LA LIGNE CLÉ À CHANGER : 'attachment' au lieu de 'inline'
