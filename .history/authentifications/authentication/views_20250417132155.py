...
class FinLocationListView(ListView):
    ...
    def get_queryset(self):
        """
        Surcharge la méthode pour ne retourner que les chambres occupées
        et optimiser la requête pour récupérer les objets liés.
        """
        # Filtre les chambres par état 'occupée'
        # Utilise select_related pour Maison et Proprietaire (relations directes ForeignKey)
        # Utilise prefetch_related pour Location et Utilisateur (relation inverse puis ForeignKey)
        #   -> prefetch_related est plus efficace pour les relations inverses ou many-to-many
        queryset = Chambre.objects.filter(etat='occupée').select_related(
            'maison',                   # Jointure sur maison
            'maison__proprietaire'      # Jointure sur proprietaire via maison
        ).prefetch_related(
    ...
    def get_context_data(self, **kwargs):
        """
        (Optionnel) Peut être utilisé pour ajouter plus de contexte.
        """
        context = super().get_context_data(**kwargs)
        context['titre_page'] = "Liste des Chambres Occupées"
        return context
