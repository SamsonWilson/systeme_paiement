...
class LocationCreateView(CreateView):
    ...
    #     return reverse('generate_pdf', kwargs={'location_id': self.object.id})
     ...
     template_name = 'accuiel/locations/location_form.html'
     def form_valid(self, form):
        # ... (votre logique pour récupérer utilisateur et chambre reste la même)
        utilisateur_id = self.kwargs.get('utilisateur_id')
        chambre_id = self.kwargs.get('chambre_id')
        utilisateur = get_object_or_404(CustomUser, id=utilisateur_id)
        chambre = get_object_or_404(Chambre, id=chambre_id)
        ...
        # Appel à super().form_valid() pour enregistrer l'objet et l'assigner à self.object
        super().form_valid(form)

        # L'ANCIENNE REDIRECTION INCORRECTE :
        # return redirect('generate_pdf', location_id=self.object.id)

        # LA NOUVELLE REDIRECTION CORRECTE :
        # On redirige vers la vue intermédiaire qui gère le téléchargement et la redirection.
        return redirect('download_and_redirect', location_id=self.object.id)def get_context_data(self, **kwargs):
        # ... (cette méthode est correcte, ne la changez pas)
        context = super().get_context_data(**kwargs)
        utilisateur_id = self.kwargs.get('utilisateur_id')
        chambre_id = self.kwargs.get('chambre_id')
        context['utilisateur'] = get_object_or_404(CustomUser, id=utilisateur_id)
...
class Download_RedirectView(View):
    ...
    def get(self, request, location_id, *args, **kwargs):
        ...
        return render(request, 'pdf/download_and_redirect.html', context)
