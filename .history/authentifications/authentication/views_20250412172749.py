...
class PaiementLoyerListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Optimisation de la requête avec prefetch_related
        ...
        # Tri par date_fin décroissante (plus récent au plus ancien)
        paiements_par_location.sort(
            key=lambda x: (
                x['date_expiree'],  # Les paiements expirés en premier
                -(x['date_fin'] or datetime.max.date()).toordinal()  # Tri décroissant par date
            )
        )
