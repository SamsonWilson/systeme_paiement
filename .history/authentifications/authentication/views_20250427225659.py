...
class ChatView(LoginRequiredMixin, View):
    ...
    #     """
    #     Gère l'affichage des messages individuels entre un utilisateur et un administrateur.
    #     """
    #     user = request.user
    ...
    #         if chat_user == user:
    #             raise Http404("Vous ne pouvez pas envoyer de messages à vous-même.")

    #         # Vérifier si l'un des utilisateurs est un admin et l'autre un locataire
    #         if (user.type_utilisateur.nom == "locataire" and chat_user.type_utilisateur.nom != "admin") or \
    #            (user.type_utilisateur.nom != "locataire" and chat_user.type_utilisateur.nom == "admin"):
    #             raise Http404("La conversation doit être entre un locataire et un administrateur.")

    #         # Charger les messages entre l'utilisateur et le destinataire
    #         messages = Message.objects.filter(
    ...
    #     # Si aucun destinataire n'est spécifié, afficher une erreur
...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if receiver_id:
            ...
            # Les admins peuvent envoyer des messages à tous les utilisateurs (locataires et autres admins)
            if user.type_utilisateur.nom == 'admin':
                messages = Message.objects.filter(
                    Q(sender=user, receiver=receiver) |
                    Q(sender=receiver, receiver=user)
                ).order_by('timestamp')
            else:
                # Les locataires peuvent envoyer des messages aux autres locataires ou à l'admin
                if receiver.type_utilisateur.nom in ['admin', 'locataire']:
                    messages = Message.objects.filter(
                        Q(sender=user, receiver=receiver) |
                        Q(sender=receiver, receiver=user)
                    ).order_by('timestamp')
                else:
                    # Si le locataire essaie de contacter un utilisateur inexistant ou une erreur
            ...
            if user.type_utilisateur.nom == 'admin':
                # Si l'utilisateur est un admin, il voit tous les messages avec tous les utilisateurs
                messages = Message.objects.filter(
                    Q(sender=user) |
                    Q(receiver=user)
                ).order_by('timestamp')
            else:
                # Si l'utilisateur est un locataire, il ne voit que les messages avec les autres locataires ou l'admin
                messages = Message.objects.filter(
                    Q(sender=user) |
                    Q(receiver=user)
                ).order_by('timestamp')
