...
from .models import Chambre, CustomUser, Location, Maison, PaiementLoyer, Proprietaire, TypeChambre, Ville, Quartier,User 
...
class ChatView(LoginRequiredMixin, FormView):  # noqa: F821
    template_name = 'accuiel/messages/chat.html'
...
class MessageListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        # Récupérer l'utilisateur avec qui on discute (chat_user)
        chat_user_id = self.request.GET.get('receiver_id')
        if chat_user_id:
            try:
        ...
        # Filtrer les messages entre l'utilisateur courant et le chat_user
        if chat_user:
            return Message.objects.filter(
                Q(sender=self.request.user, receiver=chat_user) |
                Q(sender=chat_user, receiver=self.request.user)
            ).order_by('timestamp')
        else:
            # Si aucun utilisateur spécifique n'est sélectionné, retourner une liste vide
            return Message.objects.none()

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Ajouter tous les utilisateurs pour la liste des conversations
        context['all_users'] = User.objects.exclude(id=self.request.user.id)
        # Ajouter l'utilisateur avec qui on discute (chat_user)
        chat_user_id = self.request.GET.get('receiver_id')
        if chat_user_id:
            try:
                context['chat_user'] = User.objects.get(id=chat_user_id)
            except User.DoesNotExist:
