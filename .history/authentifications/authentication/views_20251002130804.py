...
class InboxView(ListView):
    ...
    def get_queryset(self):
        # L'utilisateur actuel (on suppose que c'est un bailleur/staff)
        user = self.request.user

        # 1. On récupère toutes les maisons
        maisons = Maison.objects.all() # Ou filtrez selon vos besoins

        # 2. On prépare une liste pour stocker les données de chaque "conversation"
        conversations_data = []

        for maison in maisons:
            # 3. Pour chaque maison, on récupère les informations nécessaires
            
            # Le dernier message de la conversation de groupe
            last_message = Message.objects.filter(maison=maison).order_by('-timestamp').first()
            
            # Le nombre de messages non lus pour l'utilisateur actuel dans cette conversation
            # (Cette logique peut varier selon votre modèle Message)
            unread_count = Message.objects.filter(
                maison=maison,
                is_read=False
            ).exclude(sender=user).count()

            # La liste des locataires pour l'affichage
            locataires = maison.locataires.all() # Si vous avez une relation ManyToMany

            # 4. On ajoute toutes ces informations dans un dictionnaire
