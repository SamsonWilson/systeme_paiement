...
class CheckNewMessagesView(View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            # ex: {"maison_1": 120, "maison_3": 95}
            # où 120 et 95 sont les IDs des derniers messages vus
            
            unread_counts = {}
            for maison_id, last_seen_id in data.items():
                maison_id_int = int(maison_id.split('_')[1])
                
                count = Message.objects.filter(
                    maison_id=maison_id_int,
                    id__gt=last_seen_id # __gt = greater than
                ).count()
                unread_counts[maison_id_int] = count
...
class InboxView(ListView):
    ...
    #     # Logique pour récupérer les maisons de l'utilisateur
    #     if user.is_staff:
    #         maisons_qs = Maison.objects.all()
    #     else:
    #         maisons_qs = Maison.objects.filter(chambres__locations__utilisateur=user).distinct()

    #     # On crée une sous-requête pour trouver le dernier message de chaque maison
    #     last_message_subquery = Message.objects.filter(
    #         maison=OuterRef('pk')
    #     ).order_by('-timestamp')

    #     # On annote chaque maison avec le texte et l'expéditeur du dernier message
    #     maisons = maisons_qs.annotate(
    #         last_message_text=Subquery(last_message_subquery.values('message')[:1]),
    #         last_message_sender=Subquery(last_message_subquery.values('sender__prenom')[:1])
    #     ).order_by('nom')
